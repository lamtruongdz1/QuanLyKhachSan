...
if (isset($_POST['add_room'])) {
    ...
    if ($room_no != '') {
        ...
        if (mysqli_num_rows(mysqli_query($connection, $sql)) >= 1) {
            $response['done'] = false;
            $response['data'] = "Mã số phòng đã tồn tại";
        } else {
            ...
            if ($result) {
                $response['done'] = true;
                $response['data'] = 'Tạo phòng mới thành công';
            } else {
                $response['done'] = false;
        ...
        $response['done'] = false;
        $response['data'] = "Vui lòng nhập mã số phòng";
    }
...
if (isset($_POST['edit_room'])) {
    ...
    if ($room_no != '') {
        ...
        if ($result) {
            $response['done'] = true;
            $response['data'] = 'Chỉnh sửa phòng thành công';
        } else {
            $response['done'] = false;
            $response['data'] = "DataBase Error";
        }
        ...
        $response['done'] = false;
        $response['data'] = "Vui lòng nhập mã phòng";
    }
...
if (isset($_POST['room_type'])) {
    ...
    $result = mysqli_query($connection, $sql);
    if ($result) {
        echo "<option selected disabled>Chọn kiểu phòng</option>";
        while ($room = mysqli_fetch_assoc($result)) {
            echo "<option value='" . $room['room_id'] . "'>" . $room['room_no'] . "</option>";
        }
    } else {
        echo "<option>Hiện không có</option>";
    }
}
...
if (isset($_POST['add_employee'])) {
    ...
    if ($staff_type == '' && $shift == '' && $salary == ''){
        $response['done'] = false;
        $response['data'] = "Vui lòng nhập các mục tô đỏ";
    }else{
